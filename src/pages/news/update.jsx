...
import { reqUpdateNews, reqSchoolByRealname } from '../../api';
...
const NewsUpdate = (props) => {
  ...
  const UpdateNews = async (values) => {
    ...
    // 2. 提交添加的请求
    const newsId = props.history.location.state._id;  // 得到当前行新闻的id
    const result = await reqUpdateNews({newsObj, newsId});
    // 3. 更新列表显示
    if (result.status === 0) {
